...
def handle_command_start(message):
    global last_command
    last_command = "start"
    bot.send_message(chat_id, f"Привет, меня зовут { bot.get_my_name().name}."
...
def handle_command_help(message):
    global last_command
    last_command = "help"
    bot.send_message(chat_id, "С помощью комманды \"/get_character_names\" ты можешь получить список имен персонажей,"
...
def handle_command_help(message):
    ...
    bot.send_message(chat_id, f" Имена персонажей, информация о которых доступна боту - {character_names}")

@bot.message_handler(commands=["set_name"])
def handle_command_set_name(message):
    global last_command
    last_command = "set_name"
    bot.send_message(chat_id, "Введите имя персонажа, о котором хотели бы получить информацию:")

@bot.message_handler(commands=["cancel"])
def handle_command_cancel(message):
    global last_command
    bot.send_message(chat_id, f"Ввод параметров комманды {last_command} отменен")
    last_command = ""
...
def handle_command_get_description(message):
    ...
    if current_character_name == "":
        last_command = "set_name"
        bot.send_message(chat_id, f"Имя не заданно, используйте комманду \"/set_name\", чтобы задать имя")
    else:
        current_character_description = info.find_charracter_by_name(current_character_name)["description"]
        bot.send_message(chat_id, f"Описание персонажа {current_character_name}: {current_character_description}")
...
def handle_command_get_voiced_by(message):
    ...
    if current_character_name == "":
        ...
        bot.send_message(chat_id, f"Персонаж {current_character_name}: {current_character_voiced}")
...
def handle_text_message(message):
    ...
    if last_command == "set_name":
        if info.check_character_exists(message.text):
            ...
            bot.send_message(chat_id, f"Вы выбрали персонажа \"{current_character_name}\"")
        else:
            bot.send_message(chat_id, "к сожалению имя введено неверно,  попробуйте еще раз.")
